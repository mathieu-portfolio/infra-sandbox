...
Rectangle ObjectivesDropdownPanel::menuBounds(const UiContext& context, const UiScenarioView& scenarioView) const
{
    ...
    height += static_cast<float>(std::max<std::size_t>(1, scenarioView.definition().failureConditions.size())) * kRowSpacing;
    height += kSectionSpacing;
    ...
    if (scenarioView.currentPhase() != nullptr) {
        height += kRowSpacing;
    }
...
bool ObjectivesDropdownPanel::update(UiContext& context, const UiScenarioView& scenarioView, Vector2 mouse)
{
    ...
    if (context.state->objectivesDroplistOpen && !hudPointInFieldOrMenu(mouse, field, menuBounds(context, scenarioView))) {
        context.state->objectivesDroplistOpen = false;
    }
    return false;
}

void ObjectivesDropdownPanel::drawField(const UiContext& context, const UiScenarioView& scenarioView) const
{
    if (context.state == nullptr) {
        return;
    }
    ...
    drawHudDroplistField(layout.field, "Objectives", scenarioView.objectiveSummary(), context.state->objectivesDroplistOpen);
}

void ObjectivesDropdownPanel::drawMenu(const UiContext& context, const UiScenarioView& scenarioView) const
{
    ...
    const auto& run = scenarioView.run();
    const Rectangle menu = menuBounds(context, scenarioView);
    drawHudMenuShell(menu);
    const float progress = static_cast<float>(std::clamp(scenarioView.run().objectiveProgress, 0.0, 1.0));
    ...
    if (scenarioView.definition().failureConditions.empty()) {
    ...
    if (const ScenarioPhase* phase = scenarioView.currentPhase()) {
